...
class MinesweeperDiscreteEnv(gym.Env):
    ...
    def count_neighbour_mines(self, x, y):
        ...
        for _x in range(x - 1, x + 2):
            for _y in range(y - 1, y + 2):
                if is_valid(_x, _y, self.board_size):
    ...
    def get_next_state(self, state, x, y):
        ...
        hit_mine : (bool) true if player hit a mine
    ...
    def next_step(self, state, x, y):
        ...
        state, hit_mine = self.get_next_state(my_board, x, y)
        if hit_mine:
            self.lives -= 1
            if self.lives <= 0:
                ...
                # Lost a life but game continues
